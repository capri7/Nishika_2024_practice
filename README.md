...
- **MAE（Mean Absolute Error）**: ` 0.07895958437579434`  
  この結果は、CatBoostモデルがlightGBMモデルよりも高い精度で価格を予測できていることを示しています。
---
...
以下のグラフや学習強線です。（ここはもっとわかりやすい説明にしたいです。）
![特徴量の重要度](./catboost_learning_curve.png)  

# まとめ
マンションの取引価格では面積と建築年が非常に重要なデータになります。

# 改善点や次のステップ
次のステップではスタッキングモデルなども作成してより精度の高いモデルを作成したいと思います。

著者情報（Author）

以下に、見た目をより整えた修正版を示します。Markdownの構成を改善し、**視覚的にわかりやすい説明**にしました。
